...
function extractDeadlineStatus(html) {
  const match = html.match(/(Phase \d deadline)(?:<!-- -->)?\s*([^<]+)/i);
  if (!match) {
...
function inferNextTransitionIso(phase) {
  const mapping = {
    "Phase 2": "2026-04-08T04:00:00Z",
    "Interphase 2": "2026-04-08T16:00:00Z",
    "Phase 3": "2026-04-09T04:00:00Z",
    "Interphase 3": "2026-04-09T16:00:00Z",
    "Phase 4": "2026-04-10T04:00:00Z",
    "Interphase 4": "2026-04-10T16:00:00Z",
    "Phase 5": "2026-04-11T16:00:00Z"
  };
...
async function buildLiveData(previousHistory = []) {
  ...
  const deadline = extractDeadlineStatus(html);
  ...
  return {
    ...
    nextTransitionIso: inferNextTransitionIso(phase),
    totals: {
      raisedEth: totalRaised,
