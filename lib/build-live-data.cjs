...
function getMetricPhase(phase) {
  const interphaseMap = {
    "Interphase 1": "Phase 1",
    "Interphase 2": "Phase 2",
    "Interphase 3": "Phase 3",
    "Interphase 4": "Phase 4"
  };

  return interphaseMap[phase] ?? phase;
}
...
async function buildLiveData(previousHistory = []) {
  ...
  const metricPhase = getMetricPhase(phase);
  const nextTransitionIso = inferNextTransitionIso(phase);
  const deadline = extractDeadlineStatus(phase, nextTransitionIso);
  const totalRaisedText = extractMetric(html, `${metricPhase} total raised`) ?? "0 ETH";
  const participantsText = extractMetric(html, `${metricPhase} participants`) ?? "0";
  const transfersText = extractMetric(html, `${metricPhase} transfers`) ?? "0";
